-- Harden RLS: restrict 'sindico' to read-only for vistorias and related tables, and limit condo modifications to admins only.

-- =====================
-- VISTORIAS
-- =====================
-- Ensure idempotency by dropping both legacy and new policy names before re-creating
DROP POLICY IF EXISTS "Atualizar vistorias (não síndicos ou admins)" ON public.vistorias;
...
DROP POLICY IF EXISTS "Criar vistorias (não síndicos)" ON public.vistorias;
...
DROP POLICY IF EXISTS "Deletar vistorias (não síndicos ou admins)" ON public.vistorias;
...
-- GRUPOS_VISTORIA
-- =====================
DROP POLICY IF EXISTS "Atualizar grupos (não síndicos)" ON public.grupos_vistoria;
...
DROP POLICY IF EXISTS "Criar grupos (não síndicos)" ON public.grupos_vistoria;
...
DROP POLICY IF EXISTS "Deletar grupos (não síndicos)" ON public.grupos_vistoria;
...
-- FOTOS_VISTORIA
-- =====================
DROP POLICY IF EXISTS "Atualizar fotos (não síndicos)" ON public.fotos_vistoria;
...
DROP POLICY IF EXISTS "Criar fotos (não síndicos)" ON public.fotos_vistoria;
...
DROP POLICY IF EXISTS "Deletar fotos (não síndicos)" ON public.fotos_vistoria;
...
-- CONDOMINIOS
-- =====================
-- Remove any permissive modification policies for non-admins (ignore if not present)
...
-- Keep the existing admin-only ALL and controlled SELECT policies intact