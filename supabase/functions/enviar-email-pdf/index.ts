...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (linkError) {
      ...
      return new Response(JSON.stringify({ error: "Erro ao criar link de acesso" }), {
        status: 500,
        headers: { "Content-Type": "application/json", ...corsHeaders },
      });
    }
    ...
    const baseUrl =
      ...
      "localhost";
    const frontendBaseUrl = Deno.env.get("FRONTEND_BASE_URL") || `https://${baseUrl}.lovable.app`;
    const accessUrl = `${frontendBaseUrl.replace(/\/$/, "")}/pdf-access/${token}`;
    ...
    if (emailsCopia && Array.isArray(emailsCopia) && emailsCopia.length > 0) {
      const emailsValidosCopia = emailsCopia.filter(
        email => email && typeof email === "string" && email.trim() !== "" && email.includes("@"),
      );
    ...
    if (emailError) {
      ...
      return new Response(JSON.stringify({ error: "Erro ao enviar email", details: emailError }), {
        status: 500,
        headers: { "Content-Type": "application/json", ...corsHeaders },
      });
    }
