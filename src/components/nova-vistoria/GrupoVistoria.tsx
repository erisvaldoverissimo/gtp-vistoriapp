...
const GrupoVistoria = ({
  ...
  const atualizarChecklist = (campo: keyof ChecklistTecnico, valor: any) => {
    ...
    const novoChecklist = { ...checklistAtual, [campo]: valor };
  ...
  const tipos = checklist?.sistemaId && checklist?.elementoId ? obterTiposPorElemento(checklist.sistemaId, checklist.elementoId) : [];
  const manifestacoes = checklist?.sistemaId && checklist?.elementoId ? obterManifestacoesPorElemento(checklist.sistemaId, checklist.elementoId) : [];
            ...
            {checklist?.sistemaId && checklist?.elementoId && (
              <div className="bg-muted p-3 rounded-md">
                <div className="text-sm space-y-1">
                  <div><strong>Sistema:</strong> {sistemasDisponiveis.find(s => s.id === checklist.sistemaId)?.nome}</div>
                  <div><strong>Elemento:</strong> {elementos.find(e => e.id === checklist.elementoId)?.nome}</div>
                  {checklist.tipo && <div><strong>Tipo:</strong> {checklist.tipo}</div>}
