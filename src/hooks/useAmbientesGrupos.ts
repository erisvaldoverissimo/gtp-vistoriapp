...
export const useAmbientesGrupos = () => {
  ...
  // Ambientes padrão
  const ambientesPadrao: Ambiente[] = [
  ...
  // Grupos padrão
  const gruposPadrao: GrupoVistoria[] = [
    { id: 'default-1', nome: 'Inspeção Predial (PMUO) [ABNT NBR 5674]', dataCriacao: new Date().toISOString() },
  ...
  const obterAmbientesPorCondominio = (condominioId?: string) => {
    return ambientes.filter(ambiente => 
      !ambiente.condominioId || ambiente.condominioId === condominioId
    );
  };

  const obterGruposPorCondominio = (condominioId?: string) => {
    return grupos.filter(grupo => 
      !grupo.condominioId || grupo.condominioId === condominioId
    );
  };
