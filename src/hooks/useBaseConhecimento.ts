...
export const useBaseConhecimento = () => {
  ...
  const carregarBaseConhecimento = async () => {
    try {
      setLoading(true);
  ...
  const adicionarConhecimento = async (novoConhecimento: NovaBaseConhecimento): Promise<BaseConhecimento | null> => {
    try {
  ...
  const atualizarConhecimento = async (id: string, dados: Partial<NovaBaseConhecimento>): Promise<boolean> => {
    try {
      const { data, error } = await supabase
        .from('base_conhecimento')
        .update(dados)
  ...
  const buscarConhecimentoRelevante = async (contexto: string, categoria?: string): Promise<BaseConhecimento[]> => {
    try {
      ...
      const termos = contexto.toLowerCase().split(' ').filter(t => t.length > 3);
      if (termos.length > 0) {
