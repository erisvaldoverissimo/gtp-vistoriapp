  ...
  useEffect(() => {
    ...
    const verificarAcesso = async () => {
      try {
        ...
        const vistoriaData = linkData.vistoria;
        const grupos = (vistoriaData.grupos_vistoria || []).map((grupo: any) => ({
          id: grupo.id,
          ...
          checklist_tecnico: grupo.checklist_tecnico ? 
            (typeof grupo.checklist_tecnico === 'string' ? 
              JSON.parse(grupo.checklist_tecnico) : 
              grupo.checklist_tecnico) : 
            undefined
        }));
        ...
        await supabase
          ...
          .update({
            ...
            acessos_count: (linkData.acessos_count || 0) + 1
          })
          .eq('id', linkData.id);
          ...
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1">
              ...
              <div className="mt-1 text-sm text-blue-700">
                <p><strong>Enviado para:</strong> {linkInfo?.email_enviado_para}</p>
                <p><strong>Expira em:</strong> {new Date(linkInfo?.expires_at).toLocaleDateString('pt-BR')} às {new Date(linkInfo?.expires_at).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</p>
                <p><strong>Acessos:</strong> {linkInfo?.acessos_count || 0} vez(es)</p>
              </div>
            </div>
          </div>
        </Card>
